    ...
    if not has_industry:
        ...
        if data_df is not None:
            ...
            if st.button("🚀 Run ULTRA-FAST Analysis", type="primary", width='stretch'):
                ...
                # Word Tree Visualization - SIMPLE & AUTOMATIC
                st.markdown("### 🌳 Common Phrase Patterns")
                st.markdown("**Top conversation starters and their continuations**")
                
                # Initialize word tree visualizer
                word_tree_viz = WordTreeVisualizer(max_depth=3, min_frequency=2)
                
                # Get top 3 most common phrases automatically
                common_phrases = word_tree_viz.get_common_phrases(output_df, 'Original_Text', top_n=3)
                
                if common_phrases:
                    # Display word trees for top 3 phrases automatically
                    for idx, phrase in enumerate(common_phrases, 1):
                        try:
                            # Create network-style word tree (forward direction)
                            fig_word_tree = word_tree_viz.create_word_tree_network(
                                df=output_df,
                                text_column='Original_Text',
                                root_phrase=phrase,
                                direction='forward',
                                title=f"<b>#{idx}: Phrases starting with '{phrase}'</b>"
                            )
                            ...
                            st.warning(f"⚠️ Could not generate word tree for '{phrase}': {e}")
                else:
                    st.info("No common phrases found in the data.")
